                      ...
                      <tr key={u.id} className="hover:bg-slate-50/60 transition-colors">
                        ...
                        <td className="py-3.5 px-4">
                          ...
                          >
                            ...
                            {u.status === 'rejected' && <X className="w-3 h-3" />}
                            <span>{u.status === 'rejected' ? 'revoked' : u.status}</span>
                          </span>
                        </td>
                        ...
                        <td className="py-3.5 px-4 text-right">
                          <div className="flex items-center justify-end gap-1.5">
                            {isRootSuperAdmin ? (
                              <span className="text-[11px] text-slate-400 italic px-2 py-1 bg-slate-100 rounded-lg">
                                Protected
                              </span>
                            ) : (
                              <>
                                ...
                                {/* Remove Access Button if approved */}
                                {u.status === 'approved' && (
                                  <button
                                    onClick={() => setConfirmModal({ type: 'revoke', user: u })}
                                    className="px-2.5 py-1.5 rounded-xl bg-amber-50 hover:bg-amber-100 border border-amber-200 text-amber-800 text-[11px] font-bold transition-colors cursor-pointer inline-flex items-center gap-1"
                                    title="Remove access and lock out user"
                                  >
